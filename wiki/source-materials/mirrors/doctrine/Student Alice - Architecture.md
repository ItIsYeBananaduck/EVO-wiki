...
- Privacy-preserving
- Template-adaptive
- Bound by teacher constraints
- Non-authoritative

SA helps the student learn.
SA does not override the teacher.
SA does not report behavior.
The app reports assignment completion.
...
2. District Envelope
3. Teacher Alice (TA)
4. Teacher Local LoRA (TL)
5. Global Teacher LoRA (GTL)
6. Global Student LoRA (GSL)
7. Student Pattern Model
8. Template Mesh

Student preference influences selection,
but never overrides teacher envelope.
...
1. Deliver small concept
2. Immediate comprehension check
3. Reinforcement
4. Retention probe
5. Adjust template if needed

Small batches reduce overwhelm.
Frequent feedback improves adaptation speed.
...
Purple = reinforced understanding.
Not failure.
...
If student finds answer first:
- SA logs retrieval anchor
- Improves future lookup mapping
...
If declined:
System still reports student + concept tag.
...
Do not answer from pattern memory.
...
Confusion is treated as:
Signal for method mismatch.
